...
<div class="panel" style="display:flex;align-items:center;justify-content:flex-end;gap:12px;padding:12px 20px;">
  ...
  <form method="post" style="margin:0">
    ...
    <span class="switch prestashop-switch fixed-width-lg">
      <input type="radio" name="bo_menu_visible" id="bo_menu_visible_on" value="1"{if $fpl_checked} checked="checked"{/if} onchange="this.form.submit()">
      <label for="bo_menu_visible_on" class="radioCheck">{$fpl_t.yes|escape:'html':'UTF-8'}</label>
      <input type="radio" name="bo_menu_visible" id="bo_menu_visible_off" value="0"{if !$fpl_checked} checked="checked"{/if} onchange="this.form.submit()">
